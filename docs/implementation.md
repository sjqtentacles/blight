...
| `Glue` / `ua` | ✅ checked (F1) | the re-checker models Glue formation + the CCHM boundary reductions (`equiv_type`, re-derived independently) *and* the `transp`-over-`ua` univalence computation (`transp_glue`, forward + inverse); `ua` re-checks `Ok` in both checkers |
