...
| `blight-recheck/src/kan.rs` | 91.52% | closed from a 0% gap (Track M1): white-box unit tests exercise the constant-line fast paths, the heterogeneous Π/Σ/PathP `transp`/`hcomp` structural dispatch, derived `comp`, and `#[should_panic]` goldens for the two fail-safe `unimplemented!` arms. The remaining ~8% is the Glue-transport Kan branches (`transp`/`hcomp` over a `Glue` line) the re-checker fail-safe-panics on pending F1 increment 3, unreached by the current corpus. |
